...
TOOL_IDS = [
    int(x) for x in os.environ.get("CROUS_TOOL_IDS", "42,47").split(",") if x.strip()
]
...
def parse_listings(html: str, tool_id: int) -> list[Listing]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        # On remonte dans l'arbre HTML jusqu'à trouver un conteneur
        # "complet" (contient prix / adresse / surface).
        container = a
        text = ""
        for _ in range(8):
